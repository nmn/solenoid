...
export function getIsolatedArrowFunctionAndVars(
  ...
  const closureToInternal = Object.fromEntries(closureArr.map((c, i) => [c, `$c${i}`]));

  const paramArr = Array.from(params);
  const paramToInternal = Object.fromEntries(paramArr.map((c, i) => [c, `$p${i}`]));
...
function createFakePath<T extends t.Expression>(target: T): NodePath<T> {
  ...
  const fakeProgram = t.file(t.program([
    t.expressionStatement(target)
  ]));
