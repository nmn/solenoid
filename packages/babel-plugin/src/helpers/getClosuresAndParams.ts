...
function getParams(
	...
	const set = new Set<string>(
		Object.entries(arrowPath.scope.bindings)
...
function isClosureIdentifier(
	...
	if (t.isObjectProperty(path.parent) && !path.parent.computed) {
		return false;
	}

	// If it's a param or declared in the arrow's body, skip it
	const binding = path.scope.getBinding(name);
	if (binding?.scope === arrowPath.scope || paramsSet.has(name)) {
		return false;
	}
